...
typedef struct {
  PyObject_HEAD PyObject *io;
  xmlTextReaderPtr reader;
} ov_xml_reader_object;

static void ov_xml_reader_dealloc(ov_xml_reader_object *self) {
...
static int ov_xml_reader_init(ov_xml_reader_object *self, PyObject *args,
                              PyObject *kwds) {
  PyObject *read = NULL;
  ...
  /* Check that the "io" parameter has a "read" method, as this catches most
     errors caused by passing a wrong type of object: */
  read = PyObject_GetAttrString(self->io, "read");
  if (read == NULL) {
    Py_DECREF(self->io);
    self->io = NULL;
    PyErr_Format(PyExc_TypeError, "The 'io' parameter doesn't look like an IO "
                                  "object, doesn't have a 'read' method");
    return -1;
  }
  Py_DECREF(read);
  ...
  self->reader =
      xmlReaderForIO(ov_xml_reader_callback, NULL, self->io, NULL, NULL, 0);
  if (self->reader == NULL) {
    Py_DECREF(self->io);
    self->io = NULL;
...
static PyObject *ov_xml_reader_read(ov_xml_reader_object *self) {
  int rc = 0;
...
static PyObject *ov_xml_reader_forward(ov_xml_reader_object *self) {
  int c_type = 0;
  int rc = 0;
  ...
  for (;;) {
    ...
    if (c_type == -1) {
      ...
      Py_RETURN_TRUE;
    } else if (c_type == XML_READER_TYPE_END_ELEMENT ||
               c_type == XML_READER_TYPE_NONE) {
      Py_RETURN_FALSE;
    } else {
      rc = xmlTextReaderRead(self->reader);
      if (rc == -1) {
...
static PyObject *ov_xml_reader_node_name(ov_xml_reader_object *self) {
...
static PyObject *ov_xml_reader_empty_element(ov_xml_reader_object *self) {
  int c_empty = 0;
...
static PyObject *ov_xml_reader_get_attribute(ov_xml_reader_object *self,
                                             PyObject *name) {
  PyObject *value = NULL;
  xmlChar *c_name = NULL;
...
static PyObject *ov_xml_reader_read_element(ov_xml_reader_object *self) {
  PyObject *value = NULL;
  int c_empty = 0;
  int c_type = 0;
  int rc = 0;
  ...
  /* For empty values elements is no need to read the value. For non empty
     values we need to read the value, and check if it is NULL, as that means
...
static PyObject *ov_xml_reader_read_elements(ov_xml_reader_object *self) {
  ...
  /* This method assumes that the reader is positioned at the element that
     contains the values to read. For example if the XML document is the
     following:
     ...
     The reader should be positioned at the <list> element. The first thing we
     need to do is to check that: */
  c_type = xmlTextReaderNodeType(self->reader);
  if (c_type == -1) {
    PyErr_Format(PyExc_Exception, "Can't get current node type");
    goto error;
  ...
  /* If we are indeed posititoned in the first element, then we need to check if
     it is empty, <list/>, as we will need this later, after discarding the
     element: */
  c_empty = xmlTextReaderIsEmptyElement(self->reader);
  if (c_empty == -1) {
    PyErr_Format(PyExc_Exception, "Can't check if current element is empty");
  ...
  /* Now we need to discard the current element, as we are interested only in
     the nested <value>...</value> elements: */
  rc = xmlTextReaderRead(self->reader);
  if (rc == -1) {
    PyErr_Format(PyExc_Exception, "Can't move to next node");
    goto error;
  ...
  /* At this point, if the start element was empty, we don't need to do anything
   * else: */
  if (c_empty) {
    goto success;
  }
  ...
  /* Once all the nested <value>...</value> elements are processed the reader
     will be positioned at the closing
     </list> element, or at the end of the document. If it is the closing
     element then we need to discard it. */
...
static PyObject *ov_xml_reader_next_element(ov_xml_reader_object *self) {
  int rc;
...
static PyObject *ov_xml_reader_close(ov_xml_reader_object *self) {
  xmlFreeTextReader(self->reader);
  self->reader = NULL;
  Py_RETURN_NONE;
}
...
static PyMethodDef ov_xml_reader_methods[] = {
    {"read", (PyCFunction)ov_xml_reader_read, METH_NOARGS,
     "Reads the next XML event from the input."},

    {"forward", (PyCFunction)ov_xml_reader_forward, METH_NOARGS,
     "Jumps to the next start tag, end tag or end of document. Returns `True` "
     "if stopped at an start tag, `False` "
     "otherwise."},

    {"node_name", (PyCFunction)ov_xml_reader_node_name, METH_NOARGS,
     "Returns the name of the current element."},

    {"empty_element", (PyCFunction)ov_xml_reader_empty_element, METH_NOARGS,
     "Returns a boolean value indicating if the current element is empty."},

    {"get_attribute", (PyCFunction)ov_xml_reader_get_attribute, METH_O,
     "Gets the value of the given attribute."},

    {"read_element", (PyCFunction)ov_xml_reader_read_element, METH_NOARGS,
     "Reads a string value, assuming that the cursor is positioned at the "
     "start element that contains the value."},

    {"read_elements", (PyCFunction)ov_xml_reader_read_elements, METH_NOARGS,
     "Reads a list of string values, assuming that the cursor is positioned at "
     "the start element of the element "
     "that contains the first value."},

    {"next_element", (PyCFunction)ov_xml_reader_next_element, METH_NOARGS,
     "Jumps to the beginning of the next element."},

    {"close", (PyCFunction)ov_xml_reader_close, METH_NOARGS,
...
static PyTypeObject ov_xml_reader_type = {
    ...
    /* tp_doc            */
    "This is an utility class used to read XML documents using an streaming "
    "approach. It is intended for use by other "
    "components of the SDK. Refrain from using it directly, as backwards "
...
void ov_xml_reader_define(void) {
  ...
  PyModule_AddObject(ov_xml_module, "XmlReader",
                     (PyObject *)&ov_xml_reader_type);
}
