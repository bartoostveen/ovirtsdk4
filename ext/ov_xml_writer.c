...
static void ov_xml_writer_dealloc(ov_xml_writer_object *self) {
...
static int ov_xml_writer_callback(void *context, const char *buffer,
                                  int length) {
  PyObject *data = NULL;
  PyObject *io = NULL;
  ...
  /* Convert the buffer to a Python array of bytes and write it to the IO
   * object, using the "write" method: */
...
static int ov_xml_writer_init(ov_xml_writer_object *self, PyObject *args,
                              PyObject *kwds) {
  static char *kwlist[] = {"io", "indent", NULL};
  ...
  /* If the 'io' parameter is None then we need to create a new BytesIO object.
     Otherwise we need to check that it has 'write' method, as this catches most
     errors caused by passing a wrong type of object: */
  if (io == Py_None) {
    io = PyObject_CallObject(bytes_io_class, NULL);
    if (io == NULL) {
      PyErr_Format(PyExc_Exception, "Can't create a new instance of 'BytesIO'");
      return -1;
    }
  } else {
    write = PyObject_GetAttrString(io, "write");
    if (write == NULL) {
      PyErr_Format(PyExc_TypeError, "The 'io' parameter doesn't look like an "
                                    "IO object, doesn't have a 'write' method");
      return -1;
    }
    Py_DECREF(write);
  }
  ...
  xmlOutputBufferPtr buffer =
      xmlOutputBufferCreateIO(ov_xml_writer_callback, NULL, self->io, NULL);
  if (buffer == NULL) {
    Py_DECREF(self->io);
    self->io = NULL;
...
static PyObject *ov_xml_writer_write_start(ov_xml_writer_object *self,
                                           PyObject *name) {
  xmlChar *c_name = NULL;
  int rc = 0;
  ...
  if (rc < 0) {
    PyErr_Format(PyExc_Exception, "Can't start XML element with name '%s'",
                 c_name);
    xmlFree(c_name);
    return NULL;
  }
...
static PyObject *ov_xml_writer_write_end(ov_xml_writer_object *self) {
  int rc = 0;
...
static PyObject *ov_xml_writer_write_attribute(ov_xml_writer_object *self,
                                               PyObject *args) {
  PyObject *name = NULL;
  PyObject *value = NULL;
  int rc = 0;
  ...
  if (rc < 0) {
    PyErr_Format(PyExc_Exception,
                 "Can't write attribute with name '%s' and value '%s'", c_name,
                 c_value);
    xmlFree(c_name);
    xmlFree(c_value);
    return NULL;
...
static PyObject *ov_xml_writer_write_element(ov_xml_writer_object *self,
                                             PyObject *args) {
  PyObject *name = NULL;
  PyObject *value = NULL;
  int rc = 0;
  ...
  if (rc < 0) {
    PyErr_Format(PyExc_Exception,
                 "Can't write element with name '%s' and value '%s'", c_name,
                 c_value);
    xmlFree(c_name);
    xmlFree(c_value);
    return NULL;
...
static PyObject *ov_xml_writer_flush(ov_xml_writer_object *self) {
  int rc;
...
static PyObject *ov_xml_writer_string(ov_xml_writer_object *self) {
  PyObject *raw = NULL;
...
static PyObject *ov_xml_writer_close(ov_xml_writer_object *self) {
...
static PyMethodDef ov_xml_writer_methods[] = {
    {"write_start", (PyCFunction)ov_xml_writer_write_start, METH_O,
     "Writes the start of an element with the given name."},

    {"write_end", (PyCFunction)ov_xml_writer_write_end, METH_NOARGS,
     "Writes the end of the current element."},

    {"write_attribute", (PyCFunction)ov_xml_writer_write_attribute,
     METH_VARARGS, "Writes an attribute with the given name and value."},

    {"write_element", (PyCFunction)ov_xml_writer_write_element, METH_VARARGS,
     "Writes an element with the given name and value."},

    {"flush", (PyCFunction)ov_xml_writer_flush, METH_NOARGS,
     "Flushes this writer, sending all the pending output th the underlying IO "
     "object."},

    {"string", (PyCFunction)ov_xml_writer_string, METH_NOARGS,
     "Return the XML document text generated by this writer so far."},

    {"close", (PyCFunction)ov_xml_writer_close, METH_NOARGS,
     "Closes this writer, releasing all the resources it uses. Note that this "
     "doesn't close the IO object."},
...
static PyTypeObject ov_xml_writer_type = {
    ...
    /* tp_doc            */
    "This is an utility class used to generate XML documents using an "
    "streaming approach. It is intended for use by "
    "other components of the SDK. Refrain from using it directly, as backwards "
...
void ov_xml_writer_define(void) {
  ...
  PyModule_AddObject(ov_xml_module, "XmlWriter",
                     (PyObject *)&ov_xml_writer_type);
